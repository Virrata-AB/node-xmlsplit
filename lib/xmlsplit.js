...
XmlSplit.prototype._transform = function(chunk, encoding, done) {
  ...
  if (this._header && this._data) {
    ...
    if (!this._tag) {
      ...
      var re = this._tagName ? new RegExp('<('+this._tagName+')[\\s>]','mgi')
             : /<([\w:-]+)[\s>]/mg
      ...
      if (res !== null) {
        ...
        var addToHeader = this._data.slice(0,res.index)
        
        // Need to find all tags which have to be closed if there is a preconfigured tag that is nested
        var prependToHeaderEndTag = [...addToHeader.matchAll(/<([\w:-]+)[\s>]/mg)]
          .map(r => `</${r[1]}>`)
          .reverse()
          .join("")
        this._headerEndTag = prependToHeaderEndTag + this._headerEndTag

        this._header += addToHeader
